...
PowerShell 用のセットアップスクリプトを使用する。WSL2 を使わずに Windows 上で直接動作する。

#### 前提条件

| ソフトウェア | 必須 | 備考 |
|---|---|---|
| **Git for Windows** | 必須 | [git-scm.com](https://git-scm.com/download/win) または `winget install Git.Git` |
| **GitHub アカウント** | 推奨 | ブラウザから記録を閲覧・バックアップするために必要。[無料作成](https://github.com/signup) |

> **注意**: Python, Node.js, Ollama, Claude Code, uv などは `setup.ps1` が自動でインストールします。

#### 手順

PowerShell を**管理者として**開いて実行する（winget でのインストールに必要）。
...
> **スクリプト実行ポリシー**: `setup.ps1` が実行できない場合は、先に以下を実行してください:
> ```powershell
> Set-ExecutionPolicy -ExecutionPolicy RemoteSigned -Scope CurrentUser
> ```

`setup.ps1` が以下を自動で行います（`setup.sh` と同等の処理）:
1. システムパッケージのチェック（Python 3.12+, Node.js）— 不足時は winget でのインストール方法を案内
2. uv のインストール
3. Ollama のインストール（winget 経由）・起動
4. Claude Code のインストール
5. `.mcp.json` の生成（Windows 用パス設定）
6. Git post-commit hook のインストール
7. Python 依存関係のインストール
8. 埋め込みモデル（bge-m3）のダウンロード
9. 初回 RAG インデックスの構築
10. GitHub 連携（任意）

#### 日常的な使い方

Ollama は Windows のタスクトレイで常駐するため、通常は自動起動する。
...
### Windows: Ollama が起動しない

Ollama は Windows ではタスクトレイアプリとして動作する。スタートメニューから「Ollama」を検索して起動するか、以下を実行:
